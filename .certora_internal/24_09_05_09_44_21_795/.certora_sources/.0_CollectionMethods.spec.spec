methods {
    ...
    function RoyaltiesForValidator(uint256) external returns (LibShare.Share[]) envfree;
...
// Rule to check that royalties don't exceed the maximum allowed value
rule royaltiesDontExceedMaximum(uint256 tokenId, uint256 commission, LibShare.Share[] royalties) {
    env e;
    setRoyaltiesForValidator(e, tokenId, commission, royalties);
    LibShare.Share[] newRoyalties = getValidatorRoyalties(tokenId);
    uint256 totalRoyalty = 0;
    for (uint i = 0; i < newRoyalties.length; i++) {
        totalRoyalty += newRoyalties[i].value;
    }
    assert totalRoyalty + commission <= MAX_ROYALTIES(), "Total royalties should not exceed maximum allowed";
}
