...
contract("PoolAddress", async (accounts) => {
  ...
  it("should view and pay 1st intallment amount", async () => {
    ...
    assert.equal(
      ...
      BigNumber(loan.loanDetails.acceptedTimestamp).plus(
  ...
  it("should continue paying installments after skipping a cycle", async () => {
    ...
    assert.equal(
      loan.loanDetails.lastRepaidTimestamp,
      BigNumber(loan.loanDetails.acceptedTimestamp).plus(
        BigNumber(loan.terms.paymentCycle).multiply(2)
      )
    );
    ...
    assert.equal(
      loan.loanDetails.lastRepaidTimestamp,
      BigNumber(loan.loanDetails.acceptedTimestamp).plus(
        BigNumber(loan.terms.paymentCycle).multiply(3)
      )
    );
    ...
    assert.equal(
      loan.loanDetails.lastRepaidTimestamp,
      BigNumber(loan.loanDetails.acceptedTimestamp).plus(
        BigNumber(loan.terms.paymentCycle).multiply(4)
      )
    );
    ...
    assert.equal(
      loan.loanDetails.lastRepaidTimestamp,
      BigNumber(loan.loanDetails.acceptedTimestamp).plus(
        BigNumber(loan.terms.paymentCycle).multiply(5)
      )
    );
