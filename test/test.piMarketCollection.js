...
contract("PiMarket", async (accounts) => {
    ...
    describe("Direct Sale", () => {
        ...
        it("should let alice place piNFT on sale", async () => {
            ...
            const result = await piMarket.sellNFT(collectionContract.address, 0, 50000, "0x0000000000000000000000000000000000000000");
            assert.equal(
        ...
        it("should let bob place piNFT on sale again", async () => {
            ...
            const result = await piMarket.sellNFT(collectionContract.address, 0, 10000, "0x0000000000000000000000000000000000000000", {
                from: bob,
            });
            assert.equal(
                await collectionContract.ownerOf(0),
    ...
    describe("Auction sale", () => {
        ...
        it("should let alice place piNFT on auction", async () => {
            ...
            const tx = await piMarket.SellNFT_byBid(collectionContract.address, 1, 50000, 300, "0x0000000000000000000000000000000000000000");
            assert.equal(
        ...
        it("should let alice place piNFT on auction", async () => {
            ...
            const tx = await piMarket.SellNFT_byBid(collectionContract.address, 2, 50000, 300, "0x0000000000000000000000000000000000000000");
            assert.equal(
    ...
    describe("Swap NFTs", () => {
        ...
        it("should create a piNFT with 1000 erc20 tokens to bob", async () => {
            ...
            await collectionContract.addValidator(tokenId, validator, { from: bob });
            await sampleERC20.approve(collectionContract.address, 500, { from: validator });
            const tx = await collectionContract.addERC20(
                tokenId,
                sampleERC20.address,
        ...
        it("should let alice initiate swap request", async () => {
            ...
            const result = await piMarket.makeSwapRequest(collectionContract.address, collectionContract.address, 3, 4);
        ...
        it("should let alice initiate swap request again", async () => {
            ...
            const result = await piMarket.makeSwapRequest(collectionContract.address, collectionContract.address, 5, 4);
            const swapId = result.logs[0].args.swapId.toNumber();
