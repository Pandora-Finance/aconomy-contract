...
describe("piMarket", function () {
  ...
  describe("Direct Sale", function () {
    ...
    it("should not let bob buy nft if contract is paused", async () => {
      ...
      await expect(
        piMarket.connect(bob).BuyNFT(1, { value: 50000 })
      ).to.be.revertedWith("Pausable: paused");
  ...
  describe("Auction sale", () => {
    ...
    it("should not execute bid if contract is paused", async () => {
      ...
      await expect(
        piMarket.connect(alice).executeBidOrder(4, 2)
      ).to.be.revertedWith("Pausable: paused");
  ...
  describe("Sale Test", function () {
    ...
    it("should let bob buy piNFT", async () => {
      ...
      await expect(
        piMarket.connect(bob).BuyNFT(9, { value: 5000 })
      ).to.be.revertedWithoutReason();

      result2 = await piMarket.connect(bob).BuyNFT(9, { value: 50000 });

      await expect(
        piMarket.connect(bob).BuyNFT(9, { value: 50000 })
      ).to.be.revertedWithoutReason();
    ...
    it("should let alice execute bid order and not allow buying the NFT", async () => {
      ...
      await expect(piMarket.connect(bob).BuyNFT(10, { value: 70000 })).to.be.revertedWithoutReason()

      await piMarket.connect(alice).executeBidOrder(10, 2);
    });
